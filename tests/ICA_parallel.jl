...
tasks = tasks[1:n_chunks] # randomly selects only up to n chunks
...
elseif rank == merger  # I am merger ('ll merge the arrays)
	...
	h5open("$(results_path)/ICs_$(name_vid)_$(tot_n_vids)vids_$(ratio_denom)resize_$(frame_seq)frames.h5", "w") do file
		for (i_comp, IC) in enumerate(vid_comp)
			file["comp_$(i_comp)"] = IC
		end
	end
else # I am worker

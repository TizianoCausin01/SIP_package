...
elseif rank == master_merger # I am master merger
	...
	while task_counter_merger <= n_tasks + 1 # loops over all the processes (+1 to catch the last merger that will become available, but which we do not have to assign a new task)
		idx_free_merger = findfirst(traffic_light)
		if idx_free_merger == nothing #all mergers are busy
		...
		elseif task_counter_merger <= n_tasks # we have at least one merger available and we still have tasks to do
...
elseif in(rank, mergers) # I am merger
	...
	while stop == 0 # loops until its broken
		...
		@info "$(Dates.format(now(), "HH:MM:SS")) rank $(rank): current_data $(current_data[1])"
		...
		if current_data != -1 # if the message isn't the termination message
			...
			MPI.Send(Int32(0), comm; dest=master_merger, tag=100) # sends request to master_merger # FIXME it sends the request but the master_merger doesn't receive it

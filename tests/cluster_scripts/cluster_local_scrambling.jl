...
range_ls = parse(Int, ARGS[9])
stride_ls = parse(Int,ARGS[10])
results_folder = "$(results_path)/local_scrambling/$(name_vid)_counts_cg_$(glider_coarse_g_dim[1])x$(glider_coarse_g_dim[2])x$(glider_coarse_g_dim[3])_win_$(glider_dim[1])x$(glider_dim[2])x$(glider_dim[3])_range_$(range_ls)_stride_$(stride_ls)" # changed for ls
...
function wrapper_sampling_parallel_ls(video_path, num_of_iterations, glider_coarse_g_dim, glider_dim, range_ls, stride_ls) # changed for ls
	...
	bin_vid = whole_video_conversion(video_path) # converts a target yt video into a binarized one
        bin_vid = local_scrambling(bin_vid, range_ls, stride_ls) # changed for ls
...
elseif in(rank, mergers) # I am merger
	...
	while !stop # loops until its broken
		...
		if current_data != -1 # if the message isn't the termination message
			current_dict = wrapper_sampling_parallel_ls(joinpath(split_folder, files_names[current_data]), num_of_iterations, glider_coarse_g_dim, glider_dim, range_ls, stride_ls) # changed for ls

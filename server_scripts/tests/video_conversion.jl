...
path2vid = "output_h264.mp4"
#reader = VideoIO.openvideo(path2vid)
global frame_count=0
using VideoIO
path2vid = "/home/tcausin/data/SIP_data/virtual_treadmill_split/virtual_treadmill005.mp4" 
reader = VideoIO.openvideo(path2vid, target_format=VideoIO.AV_PIX_FMT_RGB24) # it loops until the last frame has been read
while !eof(reader)
	@info "inside here"
global frame_count += 1 # updates the count
                @info "frame $(frame_count)"
                frame = VideoIO.read(reader)  # reads the movie
		@info "type $(typeof(frame))"
end # while 
